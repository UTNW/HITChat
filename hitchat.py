...
def index():
    ...
    return render_template('index.html', dataSql=dataSql,datajwc=datajwc,usermsg=g.user, picid=str(g.user.picid), picids=picids, **context)
...
def sendyzm():
    ...
    if yzmdict[ip][0] < 3 and len(telephone) == 11:
        alycode = sendmscode()
...
def question():
    if request.method == 'GET':
        ...
        if label == '问答' or label == u'问答':
            Key10 = articleKey()
            key = Key10.generator(title+content)
